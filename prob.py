...
class Stats:
    ...
    def dealer_european_hard(self, outcome, card):
        ...
        if card >= 22:
            ...
            suma = sum([self.dealer_european_hard(outcome, card+i+2)
                       * self.count_of_cards[str(i+2)] for i in range(9)])
    ...
    def dealer_european_soft(self, outcome, card):
        ...
        if card >= 22:
            ...
            suma = sum([self.dealer_european_soft(outcome,
                                                  card+i+1)*self.count_of_cards[str(i+1)] for i in range(10)])
            self.dealer_european_soft_log[(
                outcome, card)] = suma/self.total_cards
            return suma/self.total_cards
    ...
    def dealer_american(self, outcome, card):
        ...
        if card == 10:
            suma = sum([self.dealer_european_hard(outcome, i) *
                       self.count_of_cards[str(i-10)] for i in range(12, 21)])
            self.dealer_american_log[(outcome, card)
                                     ] = suma/self.total_cards_2_to_13
            return suma/self.total_cards_2_to_13
        elif card == 1:
            suma = sum([self.dealer_european_soft(outcome, i) *
                       self.count_of_cards[str(i-11)] for i in range(12, 21)])
            self.dealer_american_log[(outcome, card)
    ...
    def stand_hard(self, outcome, card):
        ...
        if outcome == 4:
            total = self.dealer_american(
                16, card) - sum([self.dealer_american(i, card) for i in range(17, 22)])
            ...
            total = sum([self.dealer_american(i, card) for i in range(
                16, outcome)]) - sum([self.dealer_american(i, card) for i in range(outcome+1, 22)])
            self.stand_hard_log[(outcome, card)] = total
            return total
        else:
    ...
    def hit_hard(self, outcome, card):
        ...
        if outcome <= 20:
            total = sum([self.hit_stand_hard(outcome+i+2, card)*self.count_of_cards[str(i+2)] for i in range(9)])
    ...
    def hit_soft(self, outcome, card):
        ...
        if outcome <= 21:
            total = sum([self.hit_stand_soft(outcome+i+1, card)*self.count_of_cards[str(i+1)] for i in range(10)])
    ...
    def double_hard(self, outcome, card):
        ...
        if outcome <= 11:
            total = sum([self.stand_hard(outcome+2+i, card) * self.count_of_cards[str(i+2)] for i in range(9)])
            total += self.stand_soft(outcome+11, card)*self.count_of_cards["1"]
            total *= 2
            self.double_hard_log[(outcome, card)] = total/self.total_cards
            return total/self.total_cards
        elif outcome <= 21:
            total = sum([self.stand_hard(outcome+1+i, card) * self.count_of_cards[str(i+1)] for i in range(10)])*2
            self.double_hard_log[(outcome, card)] = total/self.total_cards
            return total/self.total_cards
        else:
    ...
    def double_soft(self, outcome, card):
        ...
        if outcome <= 21:
            total = sum([self.stand_soft(outcome+1+i, card) * self.count_of_cards[str(i+1)] for i in range(10)])*2
    ...
    def split(self, outcome, card):
        ...
        if outcome == 1:
            total = sum([self.stand_soft(outcome+11+i, card)*self.count_of_cards[str(i+1)] for i in range(10)])*2
            self.split_log[(outcome, card)] = total/self.total_cards
            return total/self.total_cards

        else:
            total = sum([self.hsd_hard(outcome+2+i, card) * self.count_of_cards[str(i+2)] for i in range(9)])
...
def main():
    ...
    # print(stats.perfect_move("7",["A", "A"]))
